(function () {
    ...
    function injectShortcutBar() {
        const shortcutId = 'tcm-lck-shortcut-li';

        const createShortcut = () => {
            if (document.getElementById(shortcutId)) return true;

            const currentVillageId = (window.game_data && window.game_data.village) ? window.game_data.village.id : '';
            if (!currentVillageId) return false;

            const li = document.createElement('li');
            li.id = shortcutId;
            li.className = 'kalk-shortcut-li quickbar_item';

            const link = document.createElement('a');
            link.className = 'kalk-shortcut-btn quickbar_link';
            link.href = `/game.php?village=${currentVillageId}&screen=am_farm`;
            link.title = 'Asystent Farmera';

            const img = document.createElement('img');
            img.src = 'https://dspl.innogamescdn.com/asset/45436e33/graphic/unit/unit_light.png';
            img.alt = 'LCK';
            img.width = 18;
            img.height = 18;
            ...
            const host =
                document.querySelector('#quickbar_contents') ||
                document.querySelector('#quickbar_inner') ||
                document.querySelector('#quickbar_outer');

            if (host) {
                let list = host.querySelector('ul');
                if (!list) {
                    list = document.createElement('ul');
                    list.className = 'tcm-lck-shortcut-list';
            ...
            const row = document.querySelector('#quickbar_contents tr:last-child, #quickbar_inner tr:last-child');
            if (row) {
        ...
        const tryCreate = () => createShortcut();
    ...
    window.KalkulatorFarmy.Library = (function () {
                ...
                queueLib: {
                    ...
                    addItem: function (item) {
                        ...
                        for (let i = 1; i < twLib.queues.length; i++) {
                            if (twLib.queues[i].length < twLib.queues[leastBusyQueue].length) {
                                leastBusyQueue = i;
                            }
                        }
        ...
        const getDistance = function (origin, target) {
            const a = origin.toCoord(true).x - target.toCoord(true).x;
            const b = origin.toCoord(true).y - target.toCoord(true).y;
        ...
        const timestampFromString = function (timestr) {
            ...
            const todayPattern = new RegExp(window.lang['aea2b0aa9ae1534226518faaefffdaad'].replace('%s', '([\\d+|:]+)')).exec(timestr);
            const tomorrowPattern = new RegExp(window.lang['57d28d1b211fddbb7a499ead5bf23079'].replace('%s', '([\\d+|:]+)')).exec(timestr);
            const laterDatePattern = new RegExp(window.lang['0cb274c906d622fa8ce524bcfbb7552d'].replace('%1', '([\\d+|\\.]+)').replace('%2', '([\\d+|:]+)')).exec(timestr);
            let t, date;
    ...
    window.KalkulatorFarmy.Main = (function (Library) {
        ...
        const getData = function (group, newbarbs, losses) {
            ...
            const villagesProcessor = ($html) => {
                ...
                if (mobileCheck) {
                    const table = jQuery($html).find('.overview-container > div');
                    table.each((i, el) => {
                        try {
                            ...
                            unitsElements.each((_, unitElement) => {
                                ...
                                if (img.length && span.length) {
                                    const unitType = img.attr('src').split('unit_')[1].replace('@2x.webp', '').replace('.webp', '').replace('.png', '');
                            ...
                            console.error('Błąd danych wiosek:', e);
                        }
                    });
                } else {
                    $html.find('#combined_table').find('.row_a, .row_b').filter((i, el) => $(el).find('.bonus_icon_33').length == 0).map((i, el) => {
                        ...
                        const units = $el.find('.unit-item').filter((index, element) => skipUnits.indexOf(game_data.units[index]) == -1).map((index, element) => $(element).text().toNumber()).get();
        ...
        const createPlanning = function (optionDistance, optionTime, optionMaxloot, data) {
            ...
            for (const prop in data.villages) {
                const orderedFarms = Object.keys(data.farms.farms)
                ...
                orderedFarms.forEach((el) => {
                    ...
                    const template_name = optionMaxloot && farmIndex.hasOwnProperty('max_loot') && farmIndex.max_loot ? 'b' : 'a';
                    const template = data.farms.templates[template_name];
                    const unitsLeft = lib.subtractArrays(data.villages[prop].units, template.units);
        ...
        const sendFarm = function ($this) {
            ...
            if (!farmBusy && !(Accountmanager.farm.last_click && n - Accountmanager.farm.last_click < 100)) {
                farmBusy = true;
                Accountmanager.farm.last_click = n;
                const $pb = $('#KFProgessbar');
        ...
        const runLogic = function(options) {
            ...
            const optionGroup = options.fgGroup || 0;
            const optionDistance = options.fgDistance || 25;
            const optionTime = options.fgTime || 10;
            const optionLosses = options.fgLosses || false;
            const optionMaxloot = options.fgMaxLoot ?? true;
            const optionNewbarbs = options.fgNewBarbs || false;

            UI.SuccessMessage('Skanuję wioski i przeliczam wojsko...', 1500);

            getData(optionGroup, optionNewbarbs, optionLosses).then((data) => {
                const plan = createPlanning(optionDistance, optionTime, optionMaxloot, data);
                $('.kfContent').remove();
