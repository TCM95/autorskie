...
(function() {
    ...
    // Proxy do omijania blokad CORS przy użyciu standardowego fetch()
    ...
    if (screen === 'info_player') {
        ...
        fetch(corsProxy + encodeURIComponent(twStatsConquersUrl))
            .then(res => res.text())
            .then(html => {
            ...
            .catch(err => console.error("TCM Błąd: Nie udało się pobrać dat przejęć", err));

        // 2. POKONANI Z TW STATS
        fetch(corsProxy + encodeURIComponent(twStatsUrl))
            .then(res => res.text())
            .then(html => {
                const parser = new DOMParser();
                const doc = parser.parseFromString(html, "text/html");
            ...
            .catch(err => console.error("TCM Błąd: Nie udało się pobrać statystyk", err));
    ...
    // 3. INFORMACJE O POJEDYNCZEJ WIOSCE
        ...
        fetch(corsProxy + encodeURIComponent(twStatsUrl))
            ...
            .catch(err => console.error("TCM Błąd: Nie udało się pobrać informacji o wiosce", err));
    }
})();
