(async function() {
    ...
    const STABILIZATION_URL = `${BASE_URL}skrypty/stabilizacja.js`;
    ...
    function getScriptsState() {
        try {
            ...
            console.warn('Uszkodzony stan skryptów — używam pustego stanu.', e);
            return {};
        }
    }
    ...
    async function loadModule(path) {
        ...
        if (!res.ok) throw new Error(`Nie udało się pobrać modułu: ${url}`);
            ...
            if (!themeElement && darkScriptUrl) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        themeElement.textContent = await response.text();
    ...
    try {
        ...
        if (confRes.ok) {
            ...
            if (!Array.isArray(json)) {
                ...
                Object.entries(json).forEach(([categoryName, scriptList]) => {
                    ...
                    scriptList.forEach(s => scripts.push({ ...s, category: categoryName }));
                });
            } else {
                scripts = json;
        ...
        scripts.sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''), 'pl'));
        window.TCM_UI.initPanel(scripts, categories, { getScriptsState, saveScriptState, onToggleTheme: toggleDarkTheme });
        await loadModule(EXTERNAL_MENU_URL);
        await loadActiveScripts(scripts);
        await loadModule(STABILIZATION_URL);
    } catch (e) {
